...
def portfolio(data: pd.DataFrame) -> tuple[pd.DataFrame, bool, float, float]:
    ...
    portfolio["original_hold_value"] = (
        portfolio["bid_close"] * portfolio["signal"] - portfolio["original_hold_price"]
    )
    ...
    portfolio["buy_signals"] = abs(
        portfolio["position"].where(portfolio["position"] == 1, 0)
    )
    ...
    portfolio["buy_spend"] = (portfolio["buy_signals"] * portfolio["WMA"]).cumsum()
    ...
    portfolio["sell_spend"] = (portfolio["sell_signals"] * portfolio["WMA"]).cumsum()
    ...
    portfolio["base_net_asset"] = portfolio["quote_net_asset"] / portfolio["close"]
    portfolio["drawdown"] = (
